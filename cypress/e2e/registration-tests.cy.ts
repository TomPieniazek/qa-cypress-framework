...
describe('Rolnopol app registration page tests', () => {
  ...
  it('Verify that duplicate users cannot be registered', () => {
    cy.registerUser(registrationPage.createRandomUser(), 'testUser').then(function () {
      registrationPage.visit();
      registrationPage.fillRegistrationForm(this.testUser.email, this.testUser.displayName, this.testUser.password);
      registrationPage.selectors.getCreateAccountButton().click();
      notificationPage.assertFailureNotification('User with this email already exists');
    });
  });
});
